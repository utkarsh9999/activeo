...
const Courses = () => {
  ...
  const disp = async () => {
    try {
      const file = new File([JSON.stringify(jsonContent, null, 2)], 'course_list.json', { type: 'application/json' });
      const { error } = await supabase.storage.from('accounts').upload('course_list.json', file, { upsert: true });
      if (error) {
